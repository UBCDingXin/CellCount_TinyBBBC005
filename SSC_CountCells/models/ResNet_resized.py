...
class ResNet_resized(nn.Module):
    def __init__(self, block, num_blocks, ngpu=1, num_classes=24, nc=1):
        super(ResNet_resized, self).__init__()
        self.in_planes = 64
        self.ngpu = ngpu
...
def ResNet18_resized(ngpu=1, num_classes=24):
    return ResNet_resized(BasicBlock, [2,2,2,2], ngpu=ngpu, num_classes=num_classes)

def ResNet34_resized(ngpu=1, num_classes=24):
    return ResNet_resized(BasicBlock, [3,4,6,3], ngpu=ngpu, num_classes=num_classes)
...
if __name__ == "__main__":
    ...
    net = ResNet34_resized(ngpu,1).cuda()
