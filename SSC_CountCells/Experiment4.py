"""
delete 5 continuous existing cell counts
...
# data augmentation
nfake = args.nfake
formula_dir = wd + '/data/Exp4_Formulae/'
...
for nround in range(args.NROUND):
    ...
    # filename1 = wd+"/Exp4_round"+str(nround)+"_train_intensity_vs_cellcount_beforeAugment.png"
    ...
    if args.dataAugment:
        ...
        if args.knowMC:
            ...
            start_class = deleted_classes[nround][0]
            end_class = deleted_classes[nround][-1]
            AugClass = list(np.arange(start=start_class,stop=end_class+1))
    ...
    # filename2 = wd+"/Exp4_round"+str(nround)+"_valid_intensity_vs_cellcount.png"
    # DA_intensity_analysis(valid_images, valid_cellcount, valid_stain, valid_blur, filename2)
    #
    # filename3 = wd+"/Exp4_round"+str(nround)+"_train_intensity_vs_cellcount_afterAugment_knownMC_"+str(args.knowMC)+".png"
    ...
    if args.model1 not in ['LReg', 'PReg', 'QReg']:
        save_filename = save_models_folder + 'ckpt_Exp4_CV_' + args.model1 + '(' + args.PredType1 + ')_epoch' + str(args.epochs) + '_seed' + str(args.seed) + '_Round' + str(nround) + '_CNNFlip_' + str(args.model1_flip) + '_DA_' + str(args.dataAugment) + '_OneForOne_' + str(args.one_formula_per_class) + '_knowMC_' + str(args.knowMC) + '.pth'
    ...
    print("Round [%d/%d]: valid acc %.4f, valid rmse %.4f, valid mae %.4f" % (nround + 1, args.NROUND, valid_acc_all[nround], valid_rmse_all[nround], valid_mae_all[nround]))
...
if args.EnsemblePred:
    filename_fig = "Exp4_hist_" + args.model1 + "(" + args.PredType1 + ")+" + args.model2 + "(" + args.PredType2 + ")_EnsemblePred"
    if args.dataAugment:
        filename_fig = "Exp4_hist_" + args.model1 + "(" + args.PredType1 + ")+" + args.model2 + "(" + args.PredType2 + ")_DA_EnsemblePred"
else:
    filename_fig = "Exp4_hist_" + args.model1 + "(" + args.PredType1 + ")+" + args.model2 + "(" + args.PredType2 + ")"
    if args.dataAugment:
        filename_fig = "Exp4_hist_" + args.model1 + "(" + args.PredType1 + ")+" + args.model2 + "(" + args.PredType2 + ")_DA"
