...
def train_cnn(trainloader, testloader, max_count, net, unet, path_to_ckpt=None):
    '''
    net: cnn net
    unet: pre-trained unet
    '''
    ...
    for epoch in range(resume_epoch, epochs):
        ...
        for batch_idx, batch_samples in enumerate(trainloader):
            ...
            batch_pred_counts = net(batch_recon_maps)
        ...
        _, _ = test_cnn(testloader, max_count, net, unet, verbose=True)
    #end for epoch
...
def test_cnn(testloader, max_count, net, unet, verbose=True):
    net = net.eval()
    unet = unet.eval()
